...
This is for 3D globe viewing like google earth

The near field is the radius of the globe
The pivot point is the center of the planet

Zooming is exponential starting at the near field.
Lateral movmenet is dependent on the zoom, much like the trackball. The cursor should ideally track on a single position on a globe (when zoomed in)

We have coordinates of view
We can fly to a position
We can warp to a position

*/

orb.GlobeControls = function ( object, domElement ) {
	...
	this.zoomCamera = function () {
		...
		if ( _state === STATE.TOUCH_ZOOM_PAN ) {
			...
			if ( factor !== 1.0 && factor > 0.0 ) {
				...
				//var scale = _zoomStart.y; //Math.exp(_zoomStart.y - _this.baseDistance) + _this.baseDistance;
				//console.log(scale);
				//_eye.multiplyScalar( factor );

				var ff = factor = ( _zoomEnd.y - _zoomStart.y ) * _this.zoomSpeed;

				_this._zoomFactor += ff;
				_eye.setLength(_this.baseDistance + Math.exp(_this._zoomFactor));
				console.log(_this._zoomFactor);

				if ( _this._zoomFactor > Math.log(_this.maxDistance - _this.baseDistance)) {
					_this._zoomFactor = Math.log(_this.maxDistance - _this.baseDistance);
				}
	...
	this.update = function () {
		...
		_this._zoomFactor = Math.log( _eye.length() - _this.baseDistance );
...
orb.GlobeControls.prototype = Object.create( THREE.EventDispatcher.prototype );
orb.GlobeControls.prototype.constructor = orb.GlobeControls;
...
orb.MapControls = function ( object, domElement ) {
...
orb.MapControls.prototype = Object.create( THREE.EventDispatcher.prototype );
orb.MapControls.prototype.constructor = orb.GlobeControls;
/**
 * @author axiverse / http://axiverse.com
 */
